...
const PlacedStudentsTable = ({ companyId, companyName }) => {
    ...
    useEffect(() => {
        ...
        const updatedJoinedData = filteredPlacements.map(placement => {
            const studentDetails = students.find(student => student.student_id === placement.student_id);
            return {
                ...placement,
                ...studentDetails
            };
    ...
    const placementDialogFooter = (
        <React.Fragment>
            <Button label="Cancel" icon="pi pi-times" className="p-button-danger" outlined onClick={hideDialog} />
            <Button label="Save" icon="pi pi-check" outlined onClick={savePlacement} />
        </React.Fragment>
    );
    ...
    const deletePlacement = async () => {
        try {
            ...
            const response = await axios.delete(`http://localhost:3000/api/placement/deletePlacementById/${placement.placement_id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    const handleRollNumberChange = (e) => {
        ...
        const student = students.find(student => student.rollNumber === selectedRollNumber);
        let _placement = { ...placement };
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" outlined style={{ marginRight: '5px' }} onClick={() => editPlacement(rowData)} />
                <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" outlined onClick={() => confirmDeletePlacement(rowData)} />
            </React.Fragment>
        );
    };
    ...
    const deletePlacementDialogFooter = (
        <React.Fragment>
            <Button label="No" icon="pi pi-times" className="p-button-danger" outlined onClick={hideDeletePlacementDialog} />
            <Button label="Yes" icon="pi pi-check" outlined onClick={deletePlacement} />
        </React.Fragment>
    );
        ...
        <div className="datatable-crud-demo">
            ...
            <div className="card">
                ...
                >
                    ...
                    <Column field="rollNumber" header="Roll No." sortable style={{ minWidth: '8rem' }}></Column>
                    <Column field="name" header="Student Name" sortable style={{ minWidth: '12rem' }}></Column>
                    ...
                    <Column field="salary" header="Salary" sortable style={{ minWidth: '10rem' }}></Column>
                    <Column field="placement_date" header="Placement Date" sortable style={{ minWidth: '10rem' }}></Column>
                    <Column field="offer_type" header="Offer Type" sortable style={{ minWidth: '10rem' }}></Column>
                    <Column field="offer_letter" header="Offer Letter" body={offerLetterTemplate} sortable style={{ minWidth: '10rem' }}></Column>
                    <Column field="core_non_core" header="Core/Non-Core" sortable style={{ minWidth: '10rem' }}></Column>
            ...
            <Dialog visible={placementDialog} style={{ width: '450px' }} header="Placement Details" modal className="p-fluid" footer={placementDialogFooter} onHide={hideDialog}>
                ...
                <div className="field">
                    ...
                    <Dropdown id="rollNumber" value={placement.rollNumber} onChange={(e) => handleRollNumberChange(e)} options={rollNumberOptions} placeholder='Select Roll Number' checkmark={true} />
                    {submitted && !placement.rollNumber && <small className="p-error">Roll Number is required.</small>}
                </div>
                <div className="field">
                ...
                <div className="field">
                    ...
                    <InputText id="location" value={placement.location} onChange={(e) => onInputChange(e, 'location')} required className={classNames({ 'p-invalid': submitted && !placement.location })} placeholder='Eg. Mumbai' />
                </div>
                <div className="field">
                    <label htmlFor="salary">Package</label>
                    <InputNumber id="salary" value={placement.salary} onValueChange={(e) => onInputNumberChange(e, 'salary')} mode="currency" currency="INR" currencyDisplay="code" locale="en-IN" />
                    {submitted && !placement.salary && <small className="p-error">Package is required.</small>}
                </div>
                <div className="field flex-auto">
                    <label htmlFor="placement_date">Placement Date</label>
                    <Calendar id="placement_date" value={placement.placement_date} onChange={(e) => onInputChange(e, 'placement_date')} dateFormat="dd/mm/yy" mask="99/99/9999" style={{ height: '40px' }} />
                ...
                <div className="field-checkbox align-items-center">
                    <Checkbox inputId="offer_letter" checked={placement.offer_letter} onChange={(e) => handleCheckbox(e, 'offer_letter')} />
